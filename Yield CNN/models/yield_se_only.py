...
Description — Ablation Run B: SE attention only, no CoordConv.

Extends yield_resnet_focal.py with a single addition:

    SEBlock (Squeeze-and-Excitation) — inserted inside every ResidualBlock
    after the two conv layers. SE globally pools each channel, passes through
    a small FC bottleneck, and outputs per-channel attention weights. This
    lets the network suppress uninformative channels and amplify useful ones.
    CoordConv channels are absent; this run isolates whether SE attention
    alone helps or hurts relative to the resnet-focal baseline.

Everything else — data loading, augmentation, sampler, optimizer, in_ch=1,
and loss — is identical to yield_resnet_focal.py.

Usage:
    python yield_se_only.py
